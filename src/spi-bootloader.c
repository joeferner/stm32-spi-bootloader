...
#define EEPROM_SIGNATURE_ADDRESS (DATA_EEPROM_BASE + 0x0000)
#define EEPROM_SIGNATURE_NOT_OK  0x11111111
#define EEPROM_SIGNATURE_OK      0x12345678
#define SPI                      (&hspi1)
...
void verifyProgram() {
  uint32_t crc32 = calculateAppCodeCRC32();
  if (crc32 != flashHeader.crc32) {
    fail();
    return;
  }
  eeprom_writeOkSignature();
  assertIrq(); // notify we are all done and ready
}
...
void eeprom_writeOkSignature() {
  eeprom_writeSignature(EEPROM_SIGNATURE_OK);
}

void eeprom_clearOkSignature() {
  eeprom_writeSignature(EEPROM_SIGNATURE_NOT_OK);
}
...
uint32_t calculateAppCodeCRC32() {
  const uint8_t* p;
  uint32_t crc = 0;
  uint32_t size = flashHeader.size;

  p = (uint8_t*) APP_ADDRESS;
  crc = crc ^ ~0U;
